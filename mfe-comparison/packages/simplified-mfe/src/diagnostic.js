/**
 * MFE Diagnostic Utility - Fixed version
 * 
...
function initEventLogging() {
  ...
  const logContainer = document.getElementById('mfe-events-log');
  ...
  function addLogEntry(source, action, event, data) {
    ...
    if (data && data.length > 0) {
      ...
      try {
        ...
        dataEl.textContent = "Cannot stringify data: " + e.message;
      }
  ...
  // Hijack event bus emission for all buses
  window.__MFE_EVENT_REGISTRY.forEach((events, busId) => {
    // Skip buses that are already intercepted
    if (events._intercepted) return;
    events._intercepted = true;
  ...
  // Setup observer to monitor event registry changes
  const registryObserver = setInterval(() => {
    if (window.__MFE_EVENT_REGISTRY) {
      window.__MFE_EVENT_REGISTRY.forEach((events, busId) => {
        if (!events._intercepted) {
          initEventLogging(); // Re-initialize to catch new buses
        }
      });
    }
  }, 1000);
  ...
  window._mfeRegistryObserver = registryObserver;
...
function refreshStateViewer() {
  ...
  states.forEach(({ name, state }) => {
    ...
    try {
      ...
      stateContent.textContent = "Cannot stringify state: " + e.message;
    }
